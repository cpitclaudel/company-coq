...
(defconst company-coq-abbrevs-RefMan-mod
  (list 
    ...
    '("@{assumptionkeyword} Inline @{assums}" . ("2c" . 73))
    '("End @{ident}" . ("2c" . 74))
...
(defconst company-coq-abbrevs-RefMan-ltac
  (list 
    '("do @{num} @{tacexpr}" . ("9x" . 586))
    '("repeat @{tacexpr}" . ("9x" . 587))
    '("try @{tacexpr}" . ("9x" . 588))
    '("progress @{tacexpr}" . ("9x" . 589))
    '("tryif @{tacexpr} then @{tacexpr} else @{tacexpr}" . ("9x" . 590))
    '("once @{tacexpr}" . ("9x" . 591))
    '("exactly_once @{tacexpr}" . ("9x" . 592))
    '("solve [@{tacexpr} | | @{tacexpr}]" . ("9x" . 593))
    '("idtac" . ("9x" . 594))
    '("idtac @{messagetoken}" . ("9x" . 595))
    '("fail" . ("9x" . 596))
    '("fail @{n}" . ("9x" . 597))
    '("fail @{messagetoken}" . ("9x" . 598))
    '("fail @{n} @{messagetoken}" . ("9x" . 599))
    '("gfail" . ("9x" . 600))
    '("gfail @{messagetoken}" . ("9x" . 601))
    '("gfail @{n} @{messagetoken}" . ("9x" . 602))
    '("timeout @{num} @{tacexpr}" . ("9x" . 603))
    '("time @{qstring} @{tacexpr}" . ("9x" . 604))
    '("@{qualid} @{tacarg+}" . ("9x" . 605))
    '("fun @{ident+} => @{tacexpr}" . ("9x" . 606))
    '("context @{ident} [@{cpattern}]" . ("9x" . 607))
    '("appcontext @{ident} [@{cpattern}]" . ("9x" . 608))
    '("context @{ident} [@{tacexpr}]" . ("9x" . 609))
    '("fresh @{component}" . ("9x" . 610))
    '("eval @{redexpr} in @{term}" . ("9x" . 611))
    '("type of @{term}" . ("9x" . 612))
    '("uconstr : @{term}" . ("9x" . 613))
    '("type_term @{term}" . ("9x" . 614))
    '("guard @{test}" . ("9x" . 615))
    '("abstract @{tacexpr}" . ("9x" . 616))
    '("@{Qed} export" . ("9x" . 617))
    '("Qed export @{ident+}" . ("9x" . 618))
    '("abstract @{tacexpr} using @{ident}" . ("9x" . 619))
    '("Print Ltac @{qualid}." . ("a0" . 620))
    '("Info @{num} @{tacexpr}." . ("a2" . 621))
    '("Test Info Level." . ("a2" . 622))
    '("Unset Info Level." . ("a2" . 622))
    '("Set Info Level @{num}." . ("a2" . 622))
    '("Unset Info Level @{num}." . ("a2" . 623))
    '("Set Ltac Debug." . ("a3" . 624))
    '("Test Ltac Debug." . ("a3" . 624))
    '("Unset Ltac Debug." . ("a3" . 625))))

(defconst company-coq-abbrevs-RefMan-tacex
  (list 
    '("specialize_eqs" . ("a6" . 626))))
...
(defconst company-coq-abbrevs-RefMan-syn
  (list 
    '("Print Grammar pattern." . ("bb" . 627))
    '("Open Scope" . ("bn" . 628))
    '("Close Scope" . ("bn" . 629))
    '("Local Open Scope" . ("bn" . 630))
    '("Local Close Scope" . ("bn" . 631))
    '("Global Open Scope" . ("bn" . 632))
    '("Global Close Scope" . ("bn" . 633))
    '("Arguments @{qualid} @{name}%@{scope}" . ("bo" . 634))
    '("Arguments @{qualid} : clear scopes" . ("bo" . 635))
    '("Global Arguments @{qualid} @{name}%@{scope}" . ("bo" . 636))
    '("Local Arguments @{qualid} @{name}%@{scope}" . ("bo" . 637))
    '("Bind Scope @{scope} with @{qualid}" . ("bo" . 638))
    '("type_scope" . ("bq" . 641))
    '("nat_scope" . ("bq" . 642))
    '("N_scope" . ("bq" . 643))
    '("Z_scope" . ("bq" . 644))
    '("positive_scope" . ("bq" . 645))
    '("Q_scope" . ("bq" . 646))
    '("Qc_scope" . ("bq" . 647))
    '("real_scope" . ("bq" . 648))
    '("bool_scope" . ("bq" . 649))
    ...
    '("string_scope" . ("bq" . 652))
    '("char_scope" . ("bq" . 653))
    '("Print Visibility" . ("br" . 654))
    '("Print Visibility @{scope}" . ("br" . 655))
    '("Print Scope @{scope}" . ("br" . 656))
    '("Print Scopes" . ("br" . 657))
    '("Notation @{ident} @{ident}... @{ident} @{ident} := @{term}." . ("bs" . 658))
    '("Local Notation @{ident} @{ident}... @{ident} @{ident} := @{term}." . ("bs" . 658))
    '("Notation @{ident} @{ident}... @{ident} @{ident} := @{term} (only parsing)." . ("bs" . 658))
    '("Local Notation @{ident} @{ident}... @{ident} @{ident} := @{term} (only parsing)." . ("bs" . 658))
    '("Tactic Notation @{taclevel} @{proditem}... @{proditem} := @{tac}" . ("bt" . 659))
    '("Print Grammar tactic." . ("bt" . 660))))
...
(defconst company-coq-abbrevs-Extraction
  (list 
    '("Extraction @{qualid}." . ("fb" . 665))
    '("Recursive Extraction @{qualid+}." . ("fb" . 666))
    '("Extraction \"@{file}\" @{qualid+}." . ("fb" . 667))
    '("Extraction Library @{ident}." . ("fb" . 668))
    '("Recursive Extraction Library @{ident}." . ("fb" . 669))
    '("Separate Extraction @{qualid+}." . ("fb" . 670))
    '("Extraction Language Ocaml." . ("fd" . 671))
    '("Extraction Language Haskell." . ("fd" . 672))
    '("Extraction Language Scheme." . ("fd" . 673))
    '("Set Extraction Optimize." . ("fe" . 674))
    '("Test Extraction Optimize." . ("fe" . 674))
    '("Unset Extraction Optimize." . ("fe" . 675))
    '("Set Extraction Conservative Types." . ("fe" . 676))
    '("Test Extraction Conservative Types." . ("fe" . 676))
    '("Unset Extraction Conservative Types." . ("fe" . 677))
    '("Set Extraction KeepSingleton." . ("fe" . 678))
    '("Test Extraction KeepSingleton." . ("fe" . 678))
    '("Unset Extraction KeepSingleton." . ("fe" . 679))
    '("Set Extraction AutoInline." . ("fe" . 680))
    '("Test Extraction AutoInline." . ("fe" . 680))
    '("Unset Extraction AutoInline." . ("fe" . 681))
    '("Extraction Inline @{qualid+}." . ("fe" . 682))
    '("Extraction NoInline @{qualid+}." . ("fe" . 683))
    '("Print Extraction Inline." . ("fe" . 684))
    '("Reset Extraction Inline." . ("fe" . 685))
    '("Extraction Implicit @{qualid} [@{ident+}]." . ("ff" . 686))
    '("Extract Constant @{qualid} => \"@{str}\"." . ("fg" . 687))
    '("Extract Inlined Constant @{qualid} => \"@{str}\"." . ("fg" . 688))
    '("Extract Constant @{qualid} \"@{str&}\" => \"@{str}\"." . ("fg" . 689))
    '("Extract Inductive @{qualid} => \"@{str}\" [\"@{str&}\"] @{optstring}." . ("fg" . 690))
    '("Extraction Blacklist @{ident+}." . ("fh" . 691))
    '("Print Extraction Blacklist." . ("fh" . 692))
    '("Reset Extraction Blacklist." . ("fh" . 693))))
...
(defconst company-coq-abbrevs-Polynom
  (list 
    '("ring" . ("g1" . 694))
    '("ring_simplify" . ("g1" . 695))
    '("ring [@{term+}]" . ("g1" . 696))
    '("ring_simplify [@{term+}] in @{ident}" . ("g1" . 697))
    '("field" . ("g4" . 699))
    '("field [@{term+}]" . ("g4" . 700))
    '("field_simplify" . ("g4" . 701))
    '("field_simplify [@{term+}]" . ("g4" . 702))
    '("field_simplify [@{term+}] @{term+}" . ("g4" . 703))
    '("field_simplify in @{hyp}" . ("g4" . 704))
    '("field_simplify [@{term+}] in @{hyp}" . ("g4" . 705))
    '("field_simplify [@{term+}] @{term+} in @{hyp}" . ("g4" . 706))
    '("field_simplify_eq" . ("g4" . 707))
    '("field_simplify_eq [@{term+}]" . ("g4" . 708))
    '("field_simplify_eq in @{hyp}" . ("g4" . 709))
    '("field_simplify_eq [@{term+}] in @{hyp}" . ("g4" . 710))))

(defconst company-coq-abbrevs-Nsatz
  (list 
    '("nsatz" . ("ga" . 712))
    '("nsatz with radicalmax := @{num}%N strategy := @{num}%Z parameters := @{listofvars} variables := @{listofvars}" . ("ga" . 713))))

(defconst company-coq-abbrevs-Setoid
  (list 
    '("setoid_reflexivity" . ("gm" . 714))
    '("setoid_symmetry" . ("gm" . 715))
    '("setoid_symmetry in @{ident}" . ("gm" . 715))
    '("setoid_transitivity" . ("gm" . 716))
    '("setoid_rewrite @{term}" . ("gm" . 717))
    '("setoid_rewrite @{term} in @{ident}" . ("gm" . 717))
    '("setoid_rewrite <- @{term}" . ("gm" . 719))
    '("setoid_rewrite <- @{term} at @{occs}" . ("gm" . 719))
    '("setoid_rewrite <- @{term} in @{ident}" . ("gm" . 719))
    '("setoid_rewrite <- @{term} at @{occs} in @{ident}" . ("gm" . 719))
    '("setoid_rewrite @{orientation} @{term}" . ("gm" . 720))
    '("setoid_rewrite @{orientation} @{term} at @{occs}" . ("gm" . 720))
    '("setoid_rewrite @{orientation} @{term} in @{ident}" . ("gm" . 720))
    '("setoid_rewrite @{orientation} @{term} at @{occs} in @{ident}" . ("gm" . 720))
    '("setoid_replace @{term} with @{term}" . ("gm" . 722))
    '("setoid_replace @{term} with @{term} in @{ident}" . ("gm" . 722))
    '("setoid_replace @{term} with @{term} by @{tactic}" . ("gm" . 722))
    '("setoid_replace @{term} with @{term} using relation @{term}" . ("gm" . 722))
    '("setoid_replace @{term} with @{term} in @{ident} by @{tactic}" . ("gm" . 722))
    '("setoid_replace @{term} with @{term} in @{ident} using relation @{term}" . ("gm" . 722))
    '("setoid_replace @{term} with @{term} using relation @{term} by @{tactic}" . ("gm" . 722))
    '("setoid_replace @{term} with @{term} in @{ident} using relation @{term} by @{tactic}" . ("gm" . 722))
    '("Print Instances @{$$(yas-choose-value '(\"Reflexive\" \"Symmetric\" \"Transitive\" \"Equivalence\" \"PreOrder\" \"PER\" \"Proper\")) }" . ("gn" . 723))
    '("rewrite_strat @{s}" . ("gv" . 724))
    '("rewrite_strat @{s} in @{ident}" . ("gv" . 724))))

(defconst company-coq-abbrevs-AsyncProofs
  (list 
    '("Set Suggest Proof Using" . ("gx" . 725))
    '("Test Suggest Proof Using" . ("gx" . 725))
    '("Unset Suggest Proof Using" . ("gx" . 725))))
